  ...
  Object.keys(stylesheet).forEach(function(key) {
    water.push(function(holder, callback) {
      ...
      if (filter[0] === '$' && filter[1] === '$') {
        debugRender('replace', filter, 'by', params);
        render(params, holder, callback);
      }
      else if (filter[0] === '$' && filter[1] !== '$' && filter[1] !== '?') {
        debugRender('set', filter, 'by', params);
        ...
        debugRender('change', filter, 'by', params);
        var r;
        try {
        ...
        debugRender('exec', filter, 'with', require('util').inspect(params, { showHidden: false, depth:0 }));
        try {
          if (holder instanceof Error) {
            callback(holder);
          }
  ...
  Object.keys(stylesheet).forEach(function(key) {
    water.push(function(holder, callback) {
      ...
      if (filter[filter.length - 1] === '<') {
        debugInject('render', filter, 'by', params);
        ...
        debugInject('change', filter, 'by', params);
        var r;
        try {
          if (holder instanceof Error) {
            callback(holder);
        ...
        if (isFilter(filter)) {
          debugInject('exec', filter, 'with', require('util').inspect(params, { showHidden: false, depth:0 }));
          try {
            if (holder instanceof Error) {
              callback(holder);
            }
        ...
        debugInject('recursive way', filter, ':', Object.keys(params).join(','));
        ...
        debugInject('set', filter, 'with', params);
